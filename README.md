...
The `/api/tavily` request is now `{ topic: "space" | "animals" }`; arbitrary query requests from the earlier unused trivia helper are replaced. Two bounded Tavily searches run per accepted break; one supported question is shown. There is no offline question fallback. Server-side templates, domain restrictions, and matching evidence constrain the first version; these checks are not a general-purpose fact checker.
